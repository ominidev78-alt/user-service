...
const BeneficiaryModel = {
  ...
  async listByUser(userId) {
    const res = await pool.query('SELECT * FROM beneficiaries WHERE user_id=$1 ORDER BY id DESC', [
      userId,
    ]);
    return res.rows;
  },
  async get(userId, id) {
    const res = await pool.query('SELECT * FROM beneficiaries WHERE user_id=$1 AND id=$2', [
      userId,
      id,
    ]);
    return res.rows[0];
  ...
  async update(userId, id, data) {
    ...
    if (data.pix_key !== undefined) {
      fields.push(`pix_key=$${idx++}`);
      params.push(data.pix_key);
    }
    if (data.key_type !== undefined) {
      fields.push(`key_type=$${idx++}`);
      params.push(data.key_type || null);
    }
    if (fields.length === 0) {
      const res = await pool.query('SELECT * FROM beneficiaries WHERE user_id=$1 AND id=$2', [
        userId,
