    ...
    try {
      ...
      if (!userId || !Number.isFinite(userId)) {
        ...
        if (appId && clientId) {
          ...
          if (!user) throw new HttpError(401, 'InvalidAppId', { message: 'app_id inválido ou não encontrado.' })
          if (clientId !== appId && clientId !== user.client_secret) {
            throw new HttpError(401, 'InvalidClientId', { message: 'client_id inválido ou não corresponde às credenciais.' })
          }
          userId = user.id
        } else {
          throw new HttpError(400, 'MissingUserId', { message: 'userId, app_id e client_id são obrigatórios.' })
        }
      }
      const { name, bank_name, document, pix_key, key_type } = req.body;
      if (!name || !pix_key) throw new HttpError(400, 'Name and pix_key required');
      const created = await BeneficiaryModel.create(userId, { name, bank_name, document, pix_key, key_type });
      res.json(created);
    } catch (err) {
      next(err);
    }
